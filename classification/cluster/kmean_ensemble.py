...
class EnsembleClustering(object):
    ...
    def get_final_cluster(self):
        ...
        # a = np.array([result, new_result])
        return result

    def get_cm_from_lm(self):
        self.weighted_base_clustering_default()
    ...
    def weighted_base_clustering(self):
        ...
        new_learner_weights = new_learner_weights / np.sum(new_learner_weights)
        self.label_matrix = new_label_data
        self.learner_weights = new_learner_weights
        self.learner_num = self.learner_num - 10

    def weighted_base_clustering_default(self):
        new_learner_weights = np.ones(shape=(self.learner_num, ))
        for i in range(self.learner_num):
            new_learner_weights[i] = sklearn.metrics.calinski_harabaz_score(self.data, self.label_matrix[:,i])
        new_learner_weights = new_learner_weights / np.sum(new_learner_weights)
        self.learner_weights = new_learner_weights
    ...
    def evalulate_base_learner(self, X, Y):
        aris, amis, ss, chs = [], [], [], []
        for i in range(self.learner_num):
            ari = sklearn.metrics.adjusted_rand_score(Y, self.label_matrix[:, i])
            ami = sklearn.metrics.adjusted_mutual_info_score(Y, self.label_matrix[:, i])
            s = sklearn.metrics.silhouette_score(X, self.label_matrix[:, i])
            ch = sklearn.metrics.calinski_harabaz_score(X, self.label_matrix[:, i])
            aris.append(ari)
            amis.append(ami)
            ss.append(s)
            chs.append(ch)
        print('%s:' % 'base learner')
...
def evaluate(data_path):
    ...
    for i in range(10):
        ...
        # model.evalulate_base_learner(x, y)
...
evaluate(data_path + 'Prostate_Tumor.mat')
